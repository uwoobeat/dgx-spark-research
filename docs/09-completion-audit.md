...
현행 반입 경로는 2026-09-08 D-018이다. 포털은 코드·패키지 46건과 source ZIP 8건의 두 회차이며 모든 OCI 4개와 모델 3개는 수동 반입한다. 아래 최초 회차 수집 수치는 이력으로만 해석한다.
...
| 포털 실제 양식·처리·문서 확인 | `08-quarantine-portal.md` | 코드·패키지 RAW 46건과 repository RAW 8건의 두 회차로 분리, OCI 제외; 실제 회차 정보는 비공개, 최종 반입 승인 결과는 별도 확인 |
| 포털 제출용 OCI/최소 RAW 목록 | `manifests/quarantine-*` | 모델·repository archive 배제 초안 완료; 자동 감사 제공 |
| 과거 수집 이력과 현행 경로 분류 | `manifests/quarantine-attempt-result.tsv`, `manifests/manual-oci-import.txt`, 비공개 `state/import-portal-notes.md` | 과거 10건 중 8건 수집 성공(OCI 2 + RAW 6), GLM OCI 2건은 `PROTOCOL_ERROR`. D-018 이후 모든 OCI 4건은 수동 반입. 과거 성공 OCI의 CVE 검토도 유지 |
...
1. 새 코드·패키지 회차의 46건 payload·SBOM·license·scan·packaging 결과를 대조하고, 성공 OCI 2건의 CVE 이상에 대한 조치 또는 예외 승인을 확정해야 한다. 모든 OCI 4건은 `manual-oci-import.txt`로 수동 반입하여 실제 archive 검사·checksum·승인 결과를 받아야 한다. 실제 회차 식별자와 상세 상태는 비공개 기록에서 관리한다.
...
3. 코드·패키지 회차의 RAW 46개 결과를 `quarantine-raw-sources.tsv`와 `quarantine-python-debs.tsv`에 대조하고 각 artifact의 SBOM·scan·license·산출물을 보존한다. `manual-oci-import.txt`의 모든 OCI 4개는 수동 수집·반입 전후에 target이 명시적 `linux/arm64`인지 확인한다.
...
6. `audit-import-routing.sh`로 모델·OCI가 포털 목록에 없고 코드·패키지 RAW 46개와 repository RAW 8개가 분리됐는지 확인한 뒤 2인 검토한다.
